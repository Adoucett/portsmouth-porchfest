...
- Site is live: ship finished work directly to `main` (production). Don't push half-baked WIP — confirm locally first, then push.
...
- Map UX: desktop uses a left side panel (~left third) for band info instead of popups with hover micro-interactions; mobile uses bottom-sheet popups and no zone filtering. Multiple bands at one address stack into one dot with a time-ordered set list in the panel (photo carousel when 2+ images). Zones are color-coded via GeoJSON overlays. Map also has a satellite/hybrid basemap toggle (keep dot readability on top) and a search that drives a dynamic results list, not just filtering the dots.
- Branching/deploy: `main` is production (portsmouthporchfest.org). Day-to-day work commits and pushes straight to `main`; Vercel auto-deploys. Pushes can go direct via token; keep local remote-tracking refs synced afterward.
